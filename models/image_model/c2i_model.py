...
class C2IModel(nn.Module):
    ...
    def forward(self, x, t, labels):
        ...
        if labels.ndim != 1:
            raise ValueError("C2IModel expects label tensor of shape (batch,) or (batch, 1).")
        if (labels < 0).any() or (labels >= self.num_classes).any():
            raise ValueError("Label indices must be within [0, num_classes).")
        ...
        y = self.label_embedder(labels).unsqueeze(1).to(dtype=t.dtype)
